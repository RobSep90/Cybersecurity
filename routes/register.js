...
export async function registerUser(c) {
    const body = await c.req.parseBody(); const username = body.username;
    const password = body.password; const birthdate = body.birthdate; const role = body.role;
    try {// Validate if username is an email
        if (!isValidEmail(username)) { return c.text('Invalid email address', 400); }
        // Check if the email is unique
        if (!(await isUniqueUsername(username))) { return c.text('Email already in use', 400); }
